...
- [x]* 6.1 为认证编写单元测试（scrypt/Cookie 签名/限速/回环判定）
...
- [x]* 8.1 属性测试：档案 CRUD 后节点唯一性不变量（设计正确性属性 1）
...
- [x]* 10.1 属性测试：镜像根外路径行为与上游本地实现一致（设计正确性属性 2/6）
...
- [x]* 12.1 安全回归测试：argv 含 shell 元字符时命令串保持静态字面量（设计正确性属性 8）
- [x]* 12.2 live 测试：对真实 sshd（含两跳 ProxyJump）覆盖 fs 全方法/collect/PTY
...
- [x]* 16.1 属性测试：远程会话模型可见路径全为节点侧真实路径（设计正确性属性 3）
- [x] 17. e2e：注册登录 → 添加 SSH 节点 → 添加远程工作区（含自动建目录）→ 新建会话执行命令 → 断言远端生效且日志集中于管理端
- [x] 18. 检查点 - 确保所有测试通过
...
- [x] 21. 检查点 - 一期收口：全量测试绿，手机浏览器登录并完整使用远程会话
...
- [x]* 23.1 属性测试：供给失败时无残留容器与绑定记录（设计正确性属性 7）
- [x] 24. 检查点 - 确保所有测试通过，四类节点全通
