...
export async function provisionContainer(
  ...
  const inspectExisting = await runner.run(['docker', 'inspect', '--format', '{{.Id}}\t{{.State.Running}}', name])
  if (inspectExisting.code === 0) {
    const [containerId, running] = inspectExisting.stdout.trim().split('\t')
    if (running === 'true') {
      onStage?.('ready', `容器 ${name} 已在运行`)
      return { containerId: containerId ?? name, name }
    }
    const startResult = await runner.run(['docker', 'start', name])
    if (startResult.code === 0) {
      onStage?.('ready', `容器 ${name} 已重新启动`)
      return { containerId: containerId ?? name, name }
    }
  }
  ...
  if (config.memoryMB && config.memoryMB >= 6) createArgs.push('-m', `${config.memoryMB}m`)
