...
import type { DirectoryListing, DirectoryEntry, DirectoryPickerBrowseCapability } from '@deepseek-ai/dsh-host-directory-picker'

function entry(path: string, hidden: boolean): DirectoryEntry {
  return { name: path.split('/').filter(Boolean).pop() ?? '/', path, hidden }
}

export function decorateDirectoryPicker(picker: DirectoryPicker): () => void {
  const origCapability = picker.capability.bind(picker)

  picker.capability = (): DirectoryPickerBrowseCapability => {
    return {
      kind: 'browse',
  ...
  return () => {
    picker.capability = origCapability
  }
}
