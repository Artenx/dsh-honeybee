...
export class DockerShellExecutor {
  ...
  async run(spec: ShellExecSpec): Promise<ShellRunResult> {
    const result = await this.client.exec(['bash', '-c', spec.command], spec.workdir, (spec.env ?? {}) as Record<string, string>, spec.stdin !== undefined ? Buffer.from(spec.stdin) : undefined)
...
export class DockerSubprocessRuntime {
  ...
  spawn(spec: SubprocessSpawnSpec): SubprocessHandle {
    ...
    const handle: SubprocessHandle = {
      pid: -1,
      done: promise.then((r) => ({ exitCode: r.code, signal: null as NodeJS.Signals | null })),
      kill: () => {
        void promise
      },
